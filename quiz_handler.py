...
def ask_questions(question: list, quiz_type: cat.Cat, help_count: int, act_question, win_streak: int, quiz_data, i) -> list[str]:
    ...
    while True:
        ...
        if (already_used_help and already_used_hint): print(c.info(f"\tPuska: {quiz_data[i]['hint']}"))
        ...
        if (help_count and not already_used_help and quiz_type.name == "python_learning"): 
            match help_count:
                case 2|3|4|5: print(c.info(f"{'Súgás "*": ' + c.ok(str(help_count) + 'db'):>90}"))
                case 1: print(c.info(f"{'Súgás "*": ' + c.warning(str(help_count) + 'db'):>91}"))
                case _: print(c.info(f"{'Súgás "*": ' + c.error(str(help_count) + 'db'):>91}"))
        ...
        if your_answer == "/":
            if help_count == 0:
                if (quiz_type.name == "python_learning"): print(c.warning("\tNincs több segítséged!\n"))
                else: print(c.warning("\tNincs több felezési lehetőséged!\n"))
                
                time.sleep(1.2)
                continue
            if already_used_help:
                if (quiz_type.name == "python_learning"): print(c.warning("\tMár használtál egy segítséget ennél a kérdésnél!!\n"))
                else: print(c.warning("\tMár használtál egy felezést ennél a kérdésnél!\n"))
                time.sleep(1.2)
                continue
            help_count -= 1
            ...
            continue
        if (quiz_type.name == "python_learning" and your_answer == "*"):
            if help_count == 0:
